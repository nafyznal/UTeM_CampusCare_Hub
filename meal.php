...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check last request date for this meal
...
$result = $conn2->query("SELECT Kit_Id, KitName, Description, Picture FROM kit WHERE Kit_Id LIKE 'MEAL%'");

// Map Kit_Id to local image files
$mealImages = [
    'MEAL-001' => 'images/miniMeal.jpeg',
    'MEAL-002' => 'images/sanitaryPad.jpeg',
    'MEAL-003' => 'images/toiletries.jpeg',
];

while ($row = $result->fetch_assoc()) {
    $pic      = $mealImages[$row['Kit_Id']] ?? $row['Picture'] ?? '';
    $hasPic   = !empty($pic);
    $bgStyle  = $hasPic ? "background-image: url('" . htmlspecialchars($pic) . "');" : "";
    $noImgClass = $hasPic ? "" : "meal-no-img";
?>
    <div class="meal <?php echo $noImgClass; ?>" style="<?php echo $bgStyle; ?>">
...
<div id="popup" class="popup">
    <div class="popup-content">
        ...
        <!-- Close icon — inline SVG, no file needed -->
        ...
        <!-- Status icon — pure CSS, no image file needed -->
