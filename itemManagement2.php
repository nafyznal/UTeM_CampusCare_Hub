...
if (isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (!empty($kitName)) {
        ...
        if ($conn->query($sql) === TRUE) {
            header("Location: itemManagement.php?msg=success");
            exit();
        } else {
...
if (isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    if (!empty($kitName)) {
        ...
        if ($conn->query($sql) === TRUE) {
            header("Location: itemManagement.php?msg=updated");
            exit();
        } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        header("Location: itemManagement.php?msg=deleted");
        exit();
    } else {
...
<body>
        ...
        <div class="kit-container">
            <div class="kit-card">
                <div class="kit-header">
                    ...
                    <button class="delete-kit-btn" onclick="openAddModal()">
                        Add New Kit
                    </button>
                </div>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                while($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td>
                                            <a href='#' onclick='openEditModal(" . $row['Kit_Id'] . ", \"" . htmlspecialchars($row['KitName']) . "\", " . $row['Quantity'] . ", \"" . htmlspecialchars($row['Description']) . "\")\'>Edit</a> | 
                                            <a href='itemManagement.php?delete=" . $row['Kit_Id'] . "' onclick='return confirm(\"Are you sure you want to delete this kit?\");'>Delete</a>
                                          </td>";
                                    echo "</tr>";
                                }
                            } else {
    ...
    <script>
        ...
        // Close modal clicking outside content container area 
